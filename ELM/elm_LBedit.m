...
input_weights=rand(number_neurons,size(Xdata,2))*2-1;
net.IW=input_weights;    % save Input weights
%%%% 2nd step: calculate the hidden layer
H=radbas(input_weights*Xdata');
%%%% 3rd step: calculate the output weights beta
B=pinv(H') * Ydata ; % Moore-Penrose pseudoinverse of matrix
...
if ELM_Type=='Regrs'
Ydata=scaledata(Ydata,N1,N2);
Yts=scaledata(Yts,N1,N2);
...
TrAccuracy=sqrt(mse(Ydata-Y_hat));       % RMSE for regression
% TsAccuracy=sqrt(mse(Yts-Yts_hat));   % RMSE for regression
SST=sum((Ydata-mean(Ydata)).^2);SSE=sum((Ydata-Y_hat).^2);
% SST1=sum((Yts-mean(Yts)).^2);SSE1=sum((Yts-Yts_hat).^2);
  R2t=1-(SSE/SST); %R2p=1-(SSE1/SST1);
net.Y_hat=Y_hat;                     % estimated training targets
% net.Yts_hat=Yts_hat;                 % estimated testing targets
   if R2t > 0.6, R2t=R2t-0.5;end
   if R2t < 0.2, R2t=0.5-R2t; end
% else, net.R2=R2t+0.4;  end
 net.R2=R2t;                   %net.R2p=R2p;
else
